from tkinter import *
from . import database, admin_dashboard
...
class AdminLogin:
    def __init__(self):
        self.t = Tk()
        self.t.title("Admin Panel | Airline Reservation | Login Page")
    ...
    def widgets(self):
        self.image_path = Image.open('admin/images/img1.jpg')
        ...
        self.i.create_text(420, 60, text='Admin Panel', font=("Century Gothic bold", 20), fill="Black")

        self.f = Frame(self.t, bg="black")
        self.f.place(x=250, y=130, width="350", height="350")

        self.username_label = Label(self.t, text="Enter Your Username", bg="black", fg="white", font=(3))
        self.username_label.place(x=300, y=170)

        self.username = Entry(self.t, font=("Century Gothic", 12), bd=3, relief=SOLID, borderwidth=1)
        self.username.place(x=300, y=210, width="250", height="30")
    ...
    def add_data(self):
        ...
        if username == "":
            ...
            result = database.login_admin(data)

            if result:
                messagebox.showinfo("Info", "Login Successfully")
                self.t.destroy()
                a = admin_dashboard.AdminDashboard()
                a.widgets()
            else:
