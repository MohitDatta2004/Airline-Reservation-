...
from . import database, add_aircraft
from tkinter import ttk
from PIL import Image, ImageTk


class ManageAircraft:
    def __init__(self):
        self.t = Toplevel()
        self.t.title("Manage Aircrafts | Airline Reservation | Admin Panel")
    ...
    def widgets(self):
        self.image_path = Image.open('admin/images/img7.jpg')
        ...
        self.i.create_text(430, 50, text='AIRCRAFT DETAILS', fill='Black',
                           font=("Century Gothic bold", 20))

        self.add_aircraft_button = Button(self.t, text="Add Aircraft", bg='Black', fg='White', width='20',
                                          font=("Century Gothic bold", 10), command=self.open_add_aircraft_page)
        ...
        data = database.get_aircraft_data()
        for i in data:
    ...
    def actions(self, e):
        ...
        if column == "#6":
            choice = messagebox.askyesno("Alert", "Do you really want to delete these aircraft details?")
            if choice:
                result = database.delete_aircraft_data(r_id)
                if result:
                    messagebox.showinfo("Message", "Aircraft details deleted successfully.")
                    self.t.destroy()
                    a = ManageAircraft()
                    a.widgets()
                else:
            ...
            a = add_aircraft.AddAircraft(self.tr.item(row))
            self.t.destroy()
            a.widgets()

    def open_add_aircraft_page(self):
        self.t.destroy()
        d = add_aircraft.AddAircraft()
        d.widgets()
