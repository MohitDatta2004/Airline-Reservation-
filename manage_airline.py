...
from . import database, add_airlines
...
class ManageAirline:
    def __init__(self):
        self.t = Toplevel()
        self.t.title("Manage Airlines | Airline Reservation | Admin Panel")
    ...
    def widgets(self):
        self.image_path = Image.open('admin/images/img8.jpg')
        ...
        self.add_aircraft_button = Button(self.t, text="Add Airline", bg='Black', fg='White', width='20',
                                          font=("Century Gothic bold", 10), command=self.open_add_airline_page)
        ...
        self.tr.heading("#1", text="AIRLINE ID")
    ...
    def actions(self, e):
        ...
        if column == "#6":
            choice = messagebox.askyesno("Alert", "Do you really want to delete these airline details?")
            if choice:
                result = database.delete_data(r_id)
                if result:
                    messagebox.showinfo("Message", "Airline details deleted successfully")
                    self.t.destroy()
                    a = ManageAirline()
                    a.widgets()
                else:
            ...
            a = add_airlines.AddAirlines(self.tr.item(row))
            self.t.destroy()
            a.widgets()

    def open_add_airline_page(self):
        self.t.destroy()
        g = add_airlines.AddAirlines()
        g.widgets()
